...
# Find all files in rsynccheck that end in _test.py
find rsynccheck -name "*_test.py" -print0 | while IFS= read -r -d '' TEST_FILE; do
...
find rsynccheck -name "*_test.sh" -print0 | while IFS= read -r -d '' TEST_FILE; do
  echo -e "${YELLOW}Running ${TEST_FILE}${NC}"
  bash "${TEST_FILE}"
  echo -e "${GREEN}${TEST_FILE} ran successfully${NC}"
done
